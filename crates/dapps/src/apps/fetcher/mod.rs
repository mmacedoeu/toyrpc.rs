...
impl<R: URLHint + 'static, F: Fetch> ContentFetcher<F, R> {
    ...
    fn still_syncing(address: Option<(String, u16)>) -> Box<Handler> {
        Box::new(ContentHandler::error(StatusCode::ServiceUnavailable,
                                       "Sync In Progress",
        ...
        let (new_status, handler) = {
            ...
            match status {
                ...
                Some(&mut ContentStatus::Ready(ref endpoint)) => {
                    (None, endpoint.to_async_handler(path))
                }
                ...
                Some(&mut ContentStatus::Fetching(ref fetch_control))
                    ...
                    (None, fetch_control.to_async_handler(path))
                }
                ...
                _ => {
                    ...
                    match content {
                        ...
                        None => {
                            ...
                            (None,
                             Box::new(ContentHandler::error(StatusCode::NotFound,
                                                            "Resource Not Found",
                                                            "Requested resource was not found.",
                                                            None,
                                                            self.embeddable_on.clone())) as
                             Box<Handler>)
                        }
                    }
                }
...
mod tests {
    ...
    fn should_true_if_contains_the_app() {
        ...
        let fetcher = ContentFetcher::new(FakeResolver,
                                          ...
                                          Remote::new_sync(),
                                          Client::new().unwrap());
        let handler = LocalPageEndpoint::new(path,
                                             EndpointInfo {
                                                 name: "fake".into(),
                                                 description: "".into(),
                                                 version: "".into(),
