...
fn write_response_and_check_hash(id: &str,
    ...
    // try to parse id
    let id = id.parse()
        .map_err(|_| ValidationError::InvalidContentId)?;
...
impl ContentValidator for Content {
    ...
    fn validate_and_install(&self,
                            response: fetch::Response)
                            -> Result<ValidatorResponse, ValidationError> {
        let validate = |content_path: PathBuf| {
            // Create dir
            let (_, content_path) = write_response_and_check_hash(self.id.as_str(),
                                                                  content_path.clone(),
                                                                  self.id.as_str(),
                                                                  response)?;
...
impl Dapp {
    ...
    fn find_manifest(zip: &mut zip::ZipArchive<fs::File>)
        ...
        for i in 0..zip.len() {
            ...
            let manifest = file.read_to_string(&mut manifest)
                .ok()
...
impl ContentValidator for Dapp {
    ...
    fn validate_and_install(&self,
        ...
        let validate = |dapp_path: PathBuf| {
            let (file, zip_path) = write_response_and_check_hash(self.id.as_str(),
                                                                 dapp_path.clone(),
                                                                 &format!("{}.zip", self.id),
                                                                 response)?;
...
impl fmt::Display for ValidationError {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match *self {
            ...
            ValidationError::InvalidContentId => {
                write!(f,
