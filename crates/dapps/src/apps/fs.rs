...
pub fn local_endpoint<P: AsRef<Path>>(path: P,
                                      signer_address: Option<(String, u16)>)
                                      -> Option<(String, Box<LocalPageEndpoint>)> {
    ...
    path.canonicalize()
        ...
        .and_then(|path| {
            ...
            name.map(|name| {
                         ...
                         (dapp.id,
                          Box::new(LocalPageEndpoint::new(dapp.path,
                                                          dapp.info,
                                                          PageCache::Disabled,
...
pub fn local_endpoints<P: AsRef<Path>>(dapps_path: P,
                                       signer_address: Option<(String, u16)>)
                                       -> Endpoints {
    let mut pages = Endpoints::new();
    for dapp in local_dapps(dapps_path.as_ref()) {
        pages.insert(dapp.id,
                     Box::new(LocalPageEndpoint::new(dapp.path,
                                                     dapp.info,
                                                     PageCache::Disabled,
                                                     signer_address.clone())));
