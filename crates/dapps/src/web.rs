...
use handlers::{ContentFetcherHandler, ContentHandler, ContentValidator, ValidatorResponse,
...
impl<F: Fetch> Web<F> {
    pub fn boxed(embeddable_on: Embeddable,
                 web_proxy_tokens: Arc<WebProxyTokens>,
                 remote: Remote,
                 fetch: F)
                 -> Box<Endpoint> {
...
impl ContentValidator for WebInstaller {
    ...
    fn validate_and_install(&self, response: fetch::Response) -> Result<ValidatorResponse, String> {
        ...
        let mut handler = StreamingHandler::new(response, status, mime, self.embeddable_on.clone());
        if is_html {
...
impl<F: Fetch> WebHandler<F> {
    fn extract_target_url(&self, url: Option<Url>) -> Result<String, State<F>> {
        let token_and_url = self.path
            ...
            .ok_or_else(|| {
                State::Error(ContentHandler::error(StatusCode::BadRequest,
                                                   "Invalid parameter",
        ...
        match token {
            ...
            _ => {
                return Err(State::Error(ContentHandler::error(StatusCode::BadRequest,
...
impl<F: Fetch> http::RequestMiddleware for WebHandler<F> {
