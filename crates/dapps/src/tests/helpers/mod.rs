...
pub fn init_server<F, B>(process: F,
                         io: IoHandler,
                         remote: Remote)
                         -> (ServerLoop, Arc<FakeRegistrar>)
    where F: FnOnce(ServerBuilder) -> ServerBuilder<B>,
          B: Fetch
{
    init_logger();
    ...
    let server = process(ServerBuilder::new(&dapps_path, registrar.clone(), remote))
...
pub fn serve_hosts(hosts: Option<Vec<String>>) -> ServerLoop {
    ...
    init_server(|builder| builder.allowed_hosts(hosts.into()),
                Default::default(),
                Remote::new_sync())
            .0
}
...
pub fn serve_with_registrar_and_sync() -> (ServerLoop, Arc<FakeRegistrar>) {
    init_server(|builder| builder.sync_status(Arc::new(|| true)),
                Default::default(),
                Remote::new_sync())
}
...
pub fn serve_with_registrar_and_fetch_and_threads
    (multi_threaded: bool)
     -> (ServerLoop, FakeFetch, Arc<FakeRegistrar>) {
    let fetch = FakeFetch::default();
    let f = fetch.clone();
    let (server, reg) = init_server(move |builder| builder.fetch(f.clone()),
                                    Default::default(),
                                    if multi_threaded {
                                        Remote::new_thread_per_future()
                                    } else {
                                        Remote::new_sync()
                                    });
...
impl ServerBuilder {
    ...
    pub fn new<P: AsRef<Path>>(dapps_path: P,
                               registrar: Arc<ContractClient>,
                               remote: Remote)
                               -> Self {
        ServerBuilder {
...
impl<T: Fetch> ServerBuilder<T> {
    ...
    pub fn start_unsecured_http(self,
                                addr: &SocketAddr,
                                io: IoHandler)
                                -> Result<Server, http::Error> {
        let fetch = self.fetch_client();
        Server::start_http(addr,
                           io,
                           self.allowed_hosts,
                           self.signer_address,
                           self.dapps_path,
...
impl Server {
    fn start_http<F: Fetch>(addr: &SocketAddr,
                            io: IoHandler,
                            allowed_hosts: DomainsValidation<Host>,
                            signer_address: Option<(String, u16)>,
                            dapps_path: PathBuf,
                            ...
                            fetch: F)
                            -> Result<Server, http::Error> {
        let middleware = Middleware::new(remote,
                                         signer_address,
                                         dapps_path,
                                         extra_dapps,
                                         registrar,
        ...
        http::ServerBuilder::new(io)
            ...
            .start_http(addr)
            .map(|server| Server { server: Some(server) })
    }
