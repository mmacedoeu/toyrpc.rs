...
use tests::helpers::{serve_with_registrar, serve_with_registrar_and_sync, serve_with_fetch,
...
fn should_return_502_on_hash_mismatch() {
    ...
    registrar.set_result("94f093625c06887d94d9fee0d5f9cc4aaa46f33d24d1c7e4b5237e7c37d547dd"
                             .parse()
                             .unwrap(),
                         Ok(gavcoin.clone()));
    ...
    assert!(response.body.contains("HashMismatch"),
...
fn should_return_error_for_invalid_dapp_zip() {
    ...
    let (server, fetch, registrar) = serve_with_registrar_and_fetch();
    let gavcoin = GAVCOIN_DAPP.from_hex().unwrap();
    registrar.set_result("2be00befcf008bc0e7d9cdefc194db9c75352e8632f48498b5a6bfce9f02c88e"
                             .parse()
                             .unwrap(),
                         Ok(gavcoin.clone()));
    ...
    response.assert_status("HTTP/1.1 502 Bad Gateway");
    assert!(response.body.contains("InvalidArchive"),
...
fn should_return_fetched_dapp_content() {
    ...
    let (server, fetch, registrar) = serve_with_registrar_and_fetch();
    let gavcoin = GAVCOIN_DAPP.from_hex().unwrap();
    registrar.set_result("9c94e154dab8acf859b30ee80fc828fb1d38359d938751b65db71d460588d82a"
                             .parse()
                             .unwrap(),
                         Ok(gavcoin.clone()));
...
fn should_encode_and_decode_base32() {
    ...
    let encoded = base32::encode(base32::Alphabet::Crockford,
                                 "token+https://parity.io".as_bytes());
    assert_eq!("EHQPPSBE5DM78X3GECX2YBVGC5S6JX3S5SMPY", &encoded);

    let data = base32::decode(base32::Alphabet::Crockford,
