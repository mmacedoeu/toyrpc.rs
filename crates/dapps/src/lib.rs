...
impl<F> WebProxyTokens for F
    where F: Fn(String) -> bool + Send + Sync
{
    fn is_web_proxy_token_valid(&self, token: &str) -> bool {
        self(token.to_owned())
    }
...
impl Middleware {
    ...
    pub fn new<F: Fetch + Clone>(remote: Remote,
        ...
        let endpoints = apps::all_endpoints(dapps_path,
                                            extra_dapps,
                                            signer_address.clone(),
                                            web_proxy_tokens,
                                            remote.clone(),
        ...
        let special = {
            ...
            special.insert(router::SpecialEndpoint::Api,
                           Some(api::RestApi::new(cors_domains.clone(),
                                                  &endpoints,
                                                  content_fetcher.clone())));
            special
        };

        let router = router::Router::new(signer_address, content_fetcher, endpoints, special);
...
impl http::RequestMiddleware for Middleware {
    fn on_request(&self, req: &hyper::server::Request) -> http::RequestMiddlewareAction {
        ...
        Some(signer_address) => {
            [format!("http://{}{}", HOME_PAGE, DAPPS_DOMAIN),
             ...
             format!("https://{}", address(&signer_address))]
                    .into_iter()
                    .map(|val| AccessControlAllowOrigin::Value(val.into()))
                    .collect()
        }
        None => vec![],
    }
...
mod util_tests {
    ...
    fn should_return_cors_domains() {
        ...
        assert_eq!(some,
                   vec!["http://parity.web3.site".into(),
