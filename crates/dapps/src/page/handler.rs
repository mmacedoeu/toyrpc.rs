...
impl<T: Dapp> ServedFile<T> {
    pub fn new(embeddable_on: Option<(String, u16)>) -> Self {
        ServedFile::Error(ContentHandler::error(StatusCode::NotFound,
                                                "404 Not Found",
...
impl<T: Dapp> PageHandlerWaiting for PageHandler<T> {
    fn set_uri(&mut self, uri: &Uri) {
        ...
        self.file = match *uri {
                ...
                Uri::AbsoluteUri(ref url) => self.app.file(&self.extract_path(url.path())),
                _ => None,
            }
            .map_or_else(|| ServedFile::new(self.safe_to_embed_on.clone()),
                         |f| ServedFile::File(f));
    }
}
...
impl<T: Dapp> server::Handler<HttpStream> for PageHandler<T> {
    ...
    fn on_response(&mut self, res: &mut server::Response) -> Next {
        match self.file {
            ...
            ServedFile::Error(ref mut handler) => handler.on_response(res),
        }
    }
    ...
    fn on_response_writable(&mut self, encoder: &mut Encoder<HttpStream>) -> Next {
        match self.file {
            ...
            ServedFile::File(ref mut f) => {
                match encoder.write(f.next_chunk()) {
                    Ok(bytes) => {
