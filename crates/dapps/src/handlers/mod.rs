...
pub fn add_security_headers(headers: &mut header::Headers, embeddable_on: Option<(String, u16)>) {
    ...
    if let Some(embeddable_on) = embeddable_on {
        headers.set_raw("X-Frame-Options",
                        vec![format!("ALLOW-FROM http://{}", address(&embeddable_on))
                                 .into_bytes()]);
    } else {
...
pub fn extract_url(req: &server::Request) -> Option<Url> {
    convert_uri_to_url(req.uri(), req.headers().get::<header::Host>())
}

/// Extracts URL given URI and Host header.
pub fn convert_uri_to_url(uri: &hyper::Uri, host: Option<&header::Host>) -> Option<Url> {
    match *uri {
        hyper::Uri::AbsoluteUri(ref url) => {
            match Url::from_generic_url(url.clone()) {
                Ok(url) => Some(url),
                _ => None,
            }
        }
        hyper::Uri::AbsolutePath {
            ref path,
            ref query,
        } => {
