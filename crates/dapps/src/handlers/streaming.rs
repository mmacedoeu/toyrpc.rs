...
use hyper::{header, server, Decoder, Encoder};
use hyper::mime::Mime;
...
impl<R: io::Read> http::RequestMiddleware for StreamingHandler<R> {
    fn on_request(&mut self, _request: server::Request) -> http::RequestMiddlewareAction {
        let res = server::Response::new();
...
/*
    fn on_response_writable(&mut self, encoder: &mut Encoder<HttpStream>) -> Next {
        ...
        match self.content.read(&mut self.buffer[write_pos..]) {
            ...
            Ok(read) => {
                match encoder.write(&self.buffer[..write_pos + read]) {
                    Err(e) => handle_error(e),
                    Ok(0) => Next::end(),
                    Ok(wrote) => {
