...
impl ContentHandler {
    ...
    pub fn new_embeddable(code: StatusCode,
                          content: String,
                          mimetype: Mime,
                          embeddable_on: Option<(String, u16)>)
                          -> Self {
...
impl http::RequestMiddleware for ContentHandler {
    fn on_request(&mut self, res: &mut server::Request) -> http::RequestMiddlewareAction {
        res.set_status(self.code);
        res.headers_mut()
            .set(header::ContentType(self.mimetype.clone()));
