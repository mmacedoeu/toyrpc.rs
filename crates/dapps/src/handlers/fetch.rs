...
use hyper::{server, Decoder, Encoder};
use hyper::Uri;
...
impl FetchControl {
    ...
    pub fn to_async_handler(&self,
                            path: EndpointPath)
                            -> Box<server::Handler + Send> {
...
impl Errors {
    fn download_error<E: fmt::Debug>(&self, e: E) -> ContentHandler {
        ContentHandler::error(StatusCode::BadGateway,
                              "Download Error",
    ...
    fn timeout_error(&self) -> ContentHandler {
        ContentHandler::error(StatusCode::GatewayTimeout,
                              "Download Timeout",
    ...
    fn method_not_allowed(&self) -> ContentHandler {
        ContentHandler::error(StatusCode::MethodNotAllowed,
                              "Method Not Allowed",
...
impl<H: ContentValidator, F: Fetch> ContentFetcherHandler<H, F> {
    pub fn new(url: String,
        ...
        ContentFetcherHandler {
            ...
            path: path,
            errors: Errors { embeddable_on: embeddable_on },
        }
    }
    ...
    fn fetch_content(&self,
        ...
        let errors = self.errors.clone();
        self.remote
            .spawn_with_timeout(|| future, Duration::from_secs(FETCH_TIMEOUT), move || {
