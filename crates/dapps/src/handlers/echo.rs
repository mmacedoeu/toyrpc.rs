...
impl http::RequestMiddleware for EchoHandler {
    fn on_request(&mut self, _: server::Request) -> http::RequestMiddlewareAction {
        Next::read()
    }
    ...
    fn on_request_readable(&mut self, decoder: &mut Decoder) -> Next {
        match decoder.read_to_string(&mut self.content) {
            Ok(0) => {
                self.handler = Some(ContentHandler::ok(self.content.clone(),
