...
impl Url {
    ...
    pub fn from_generic_url(raw_url: url_lib::Url) -> Result<Url, String> {
        ...
        let port = raw_url
            ...
            .ok_or_else(|| format!("Unknown port for scheme: `{}`", raw_url.scheme()))?;
        let host = raw_url
            .host()
            .ok_or_else(|| {
                            "Valid host, because only data:, mailto: protocols does not have host."
                                .to_owned()
                        })?
            .to_owned();
        let path = raw_url
            .path_segments()
            .ok_or_else(|| {
                            "Valid path segments. In HTTP we won't get cannot-be-a-base URLs"
                                .to_owned()
                        })?
...
mod test {
    ...
    fn test_not_empty_username() {
        let user = Url::parse("http://john:pass@example.com")
            .unwrap()
            .username;
    ...
    fn test_not_empty_password() {
        let pass = Url::parse("http://michael:pass@example.com")
            .unwrap()
            .password;
